...
describe('client.getSlidingWindowScoresBulk([SCORE_REQUESTS])', function() {
  let client;
  ...
  const WINDOW_NUMBER = 6;
  const STEP_SIZE = Math.ceil((TEST_TO - TEST_FROM) / WINDOW_NUMBER);
  ...
  it('getSlidingWindowScoresBulk: Should succeed on valid parameters', () => {
    const SCORE_REQUEST = [
      {
        id: GENERATOR_NAME,
        test_from: TEST_FROM,
    ...
    return client.getSlidingWindowScoresBulk(SCORE_REQUEST)
      .then((res) => {
  ...
  it('getSlidingWindowScoresBulk: Should handle mixed result', () => {
    const SCORE_REQUEST = [
      {
        id: GENERATOR_NAME,
        test_from: TEST_FROM,
  ...
  it('getSlidingWindowScoresBulk: Should fail when empty array', () => {
    const SCORE_REQUEST = [];
    try {
      return client.getSlidingWindowScoresBulk(SCORE_REQUEST);
    }
