...
describe('client.getGeneratorContextOperations(<generatorId>, from, to)', function() {
  ...
  it('should retrieve all context operations', function() {
    return client.getGeneratorContextOperations(generatorId)
      .then((operations) => {
        expect(_.first(operations)).to.be.deep.equal({ agentName: agentId2, context: _.first(CONFIGURATION_1_OPERATIONS_2).context, timestamp: _.first(CONFIGURATION_1_OPERATIONS_2).timestamp });
        expect(_.last(operations)).to.be.deep.equal({ agentName: agentId2, context: _.last(CONFIGURATION_1_OPERATIONS_2).context, timestamp: _.last(CONFIGURATION_1_OPERATIONS_2).timestamp });
        ...
        const formattedExpectedResult = CONFIGURATION_1_OPERATIONS_2.slice(0, 1000)
          .map((operation) => ({ agentName: agentId2, context: operation.context, timestamp: operation.timestamp }));
  ...
  it('should only retrieve the operations after the given lower bound', function() {
    ...
    return client.getGeneratorContextOperations(generatorId, lowerBound)
      .then((operations) => {
        ...
        expect(operations[0]).to.be.deep.equal({ agentName: agentId2, context: expectedOperations2[0].context, timestamp: expectedOperations2[0].timestamp });
        expect(operations[1]).to.be.deep.equal({ agentName: agentId, context: CONFIGURATION_1_OPERATIONS_1[1].context, timestamp: CONFIGURATION_1_OPERATIONS_1[1].timestamp });
        expect(operations[2]).to.be.deep.equal({ agentName: agentId2, context: expectedOperations2[1].context, timestamp: expectedOperations2[1].timestamp });
        expect(operations[3]).to.be.deep.equal({ agentName: agentId2, context: expectedOperations2[2].context, timestamp: expectedOperations2[2].timestamp });
  ...
  it('should only retrieve the operations before the given upper bound', function() {
    ...
    return client.getGeneratorContextOperations(generatorId, undefined, 1464600406)
      .then((operations) => {
        ...
        expect(operations[operations.length - 1]).to.be.deep.equal({ agentName: agentId2, context: expectedOperations2[expectedOperations2.length - 1].context, timestamp: expectedOperations2[expectedOperations2.length - 1].timestamp });
        expect(operations[operations.length - 2]).to.be.deep.equal({ agentName: agentId2, context: expectedOperations2[expectedOperations2.length - 2].context, timestamp: expectedOperations2[expectedOperations2.length - 2].timestamp });
        expect(operations[operations.length - 3]).to.be.deep.equal({ agentName: agentId, context: CONFIGURATION_1_OPERATIONS_1[0].context, timestamp: CONFIGURATION_1_OPERATIONS_1[0].timestamp });
        expect(operations[operations.length - 4]).to.be.deep.equal({ agentName: agentId2, context: expectedOperations2[expectedOperations2.length - 3].context, timestamp: expectedOperations2[expectedOperations2.length - 3].timestamp });
  ...
  it('should only retrieve the operations between the desired bounds', function() {
    ...
    return client.getGeneratorContextOperations(generatorId, lowerBound, upperBound)
      .then((operations) => {
        ...
        expect(operations).to.be.deep.equal(expectedOperations.map((operation) => ({ agentName: agentId2, context: operation.context, timestamp: operation.timestamp })));
      });
  });
});
