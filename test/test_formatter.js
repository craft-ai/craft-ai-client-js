...
const EXPECTATIONS_DIR = path.join(
  __dirname,
  'data/interpreter-test-suite/format_decision_rules'
);
      ...
      _.each(expectations, ({ title, expectation, rules }) => {
        it(title, function() {
          if (expectation.error) {
            ...
            expect(interpreter.formatDecisionRules(rules)).to.be.deep.equal(
              expectation.string
            );
          }
        });
...
describe('interpreter.formatProperty', () => {
  describe('on generated date types, works from a Time instance', () => {
    it('From \'2018-08-07T16:06:06+0200\'', () => {
      ...
      expect(interpreter.formatProperty('month_of_year', date)).to.be.equal(
        'Aug'
      );
      expect(interpreter.formatProperty('day_of_month', date)).to.be.equal(
        '07'
      );
      expect(interpreter.formatProperty('day_of_week', date)).to.be.equal(
        'Tue'
      );
      expect(interpreter.formatProperty('time_of_day', date)).to.be.equal(
        '16:06:06'
      );
    });
